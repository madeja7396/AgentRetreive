...
- `result.minijson.v2.schema.json`: 検索結果出力契約（capability freshness 追加）
- `dataset_manifest.v1.schema.json`: データセット来歴契約
- `experiment_run_record.v1.schema.json`: 実験記録契約
- `experiment_run_record.v2.schema.json`: 実験記録契約（e2e/micro 含む拡張）
...
- `run_constraints.v2.schema.json`: 実行制約定義（再現性トレランス含む）
