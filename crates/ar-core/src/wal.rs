...
pub enum WalEntry {
    ...
    Upsert { doc_id: u32, path: String, content_hash: String, tokens: Vec<String> },
    /// Remove a document
    Delete { doc_id: u32 },
...
impl WalEntry {
    ...
    pub fn serialize(&self) -> String {
        match self {
            WalEntry::Upsert { doc_id, path, content_hash, tokens } => {
    ...
    pub fn deserialize(line: &str) -> Option<Self> {
        ...
        match parts.get(0)? {
            &"UPSERT" if parts.len() >= 5 => {
                ...
                Some(WalEntry::Upsert { doc_id, path, content_hash, tokens })
            }
...
impl WalManager {
    ...
    pub fn new<P: AsRef<Path>>(wal_path: P, snapshot_dir: P) -> io::Result<Self> {
    ...
    pub fn create_snapshot_marker(&mut self) -> io::Result<()> {
        ...
        self.append(&WalEntry::Snapshot { version: self.current_version })?;
    ...
    pub fn compact(&mut self) -> io::Result<PathBuf> {
        self.current_version += 1;
        let snapshot_path = self.snapshot_dir.join(format!("snapshot_v{}.wal", self.current_version));
